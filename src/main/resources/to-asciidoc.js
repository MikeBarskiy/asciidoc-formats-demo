...
var toAsciidoc = function (string, document) {

  string = string.replace(nbspRegex, " ");

  var all = document.createElement("div");
  //all.innerHTML = string;
  all.textContent = string
  //throw new Error(all.textContent);
  ...
  for (var i = 0; i < codes.length; i++) {
    var code = codes[i];
    var innerCode = strip(code.textContent);
  ...
  function replaceEls(html, elProperties) {
    var pattern = elProperties.type === 'void' ? '<' + elProperties.tag
            + '\\b([^>]*)\\/?>' : '<' + elProperties.tag
            + '\\b([^>]*)>([\\s\\S]*?)<\\/' + elProperties.tag + '>',
  ...
  string = string.replace(/<pre\b[^>]*>`([\s\S]*?)`<\/pre>/gi,
  ...
  function replaceLists(html) {
    ...
    html = html.replace(/<(ul|ol)\b[^>]*>([\s\S]*?)<\/\1>/gi,
        function (str, listType, innerHTML) {
          ...
          for (i = 0, len = lis.length; i < len; i++) {
            if (lis[i]) {
              ...
              lis[i] = lis[i].replace(/\s*<li[^>]*>([\s\S]*)/i,
                  function (str, innerHTML) {
